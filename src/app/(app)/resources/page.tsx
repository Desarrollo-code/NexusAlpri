...
import { Search, UploadCloud, ArchiveX, Loader2, AlertTriangle, Trash2, Edit, Save, List, KeyRound, Pin, PinOff, MoreVertical, Folder, FileText, Video, Info, FileQuestion, LayoutGrid, Eye, Download, ChevronRight, Home } from 'lucide-react';
...
const getIconForType = (type: AppResourceType['type']) => {
    switch (type) {
      ...
      case 'MANUAL': return <FileText className="h-5 w-5 text-indigo-500" />;
...
const getPreviewIconForType = (type: AppResourceType['type']) => {
    ...
    switch (type) {
        ...
        case 'MANUAL': return <FileText className={cn(className, "text-indigo-500/80")} />;
        case 'POLICY': return <FileText className={cn(className, "text-red-500/80")} />;
