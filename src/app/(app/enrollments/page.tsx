...
function EnrollmentsPageComponent() {
  ...
  useEffect(() => {
      ...
      } else if (!isLoadingCourses && courses.length > 0) {
          // Si no hay curso seleccionado, pero ya cargaron los cursos, seleccionamos el primero.
          router.replace(`${pathname}?${createQueryString({ courseId: courses[0].id, page: 1, search: null })}`);
      }
  }, [selectedCourseId, fetchCourseDetails, courses, isLoadingCourses, router, pathname, createQueryString]);
