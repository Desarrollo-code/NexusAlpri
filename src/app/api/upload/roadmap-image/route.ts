// src/app/api/upload/roadmap-image/route.ts
  ...
  try {
    ...
    if (error) {
        if (error.message.includes("The related resource does not exist")) {
             return NextResponse.json({ success: false, message: "Error de configuración de Supabase: El bucket 'roadmap_images' no existe." }, { status: 500 });
        }
        throw new Error(`Error generando URL firmada: ${error.message}`);
    }
    
    const publicUrl = supabaseAdmin.storage.from('roadmap_images').getPublicUrl(finalFilename).data.publicUrl;
    ...
    console.error('Error al procesar la subida de imagen de la hoja de ruta:', e);
