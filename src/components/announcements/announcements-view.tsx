    ...
    <div className="space-y-6">
      {user?.role !== 'STUDENT' && (
        <Tabs value={activeFilter} onValueChange={handleFilterChange}>
          <TabsList className="w-full bg-card/10 backdrop-blur-sm border border-white/20">
      ...
      {isLoading ? (
        <div className="flex justify-center items-center py-12"><Loader2 className="h-8 w-8 animate-spin text-white" /></div>
      ) : error ? (
          ...
          {totalPages > 1 && (
            <Pagination>
              <PaginationContent>
                ...
                <PaginationItem><span className="text-sm p-2 font-semibold text-center text-white">Página {currentPage} de {totalPages}</span></PaginationItem>
        ...
        <div className="text-center py-12 border-2 border-dashed border-white/30 rounded-lg bg-black/20 text-white/80">
          <Megaphone className="mx-auto h-12 w-12 mb-4" /><h3 className="text-xl font-semibold mb-2">No hay anuncios</h3><p>No se encontraron anuncios que coincidan con el filtro seleccionado.</p>
        </div>
      )}
