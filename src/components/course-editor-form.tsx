...
import type { Course as AppCourse, Module as AppModule, Lesson as AppLesson, LessonType, CourseStatus, ContentBlock } from '@/types';
...
// Nota: crypto.randomUUID() garantiza unicidad real y evita colisiones.
const generateTempId = (prefix: string) => {
  // Fallback por si algún entorno muy viejo no tiene crypto.randomUUID
  const uid =
          ...
          <AccordionItem value={`item-${module.id}`} className="border-0">
            <div className="flex items-center px-4 py-2" {...provided.dragHandleProps}>
              ...
              <Button
                variant="ghost"
                size="icon"
                className="h-8 w-8 text-destructive ml-2 shrink-0"
        ...
        case 'TEXT':
          return (
            <RichTextEditor
              ...
              placeholder="Escribe aquí texto o pega un enlace..."
              rows={4}
              disabled={isSaving}
            />
          );
        ...
        case 'FILE':
          return (
            <Input
              value={block.content}
              onChange={e => onUpdate('content', e.target.value)}
              placeholder="URL del archivo (PDF, imagen, etc.)"
              disabled={isSaving}
            />
          );
        case 'QUIZ':
...
export function CourseEditor({ courseId }: { courseId: string }) {
  ...
  const handleAddBlock = (moduleIndex: number, lessonIndex: number, type: LessonType) => {
    const newBlock: ContentBlock = {
      ...
      // Si es QUIZ, crea objeto con id único
      quiz: type === 'QUIZ'
        ? { id: generateTempId('quiz'), title: 'Nuevo Quiz', description: '', questions: [] }
        : undefined
    };
    setCourse(prev => {
      if (!prev) return null;
  ...
  const handleRemoveBlock = (moduleIndex: number, lessonIndex: number, blockIndex: number) => {
    setCourse(prev => {
      ...
      newLessons[lessonIndex] = { ...newLessons[lessonIndex], contentBlocks: newBlocks };
      // ✅ FIX: asignar lessons: newLessons (antes estaba mal)
      newModules[moduleIndex] = { ...newModules[moduleIndex], lessons: newLessons };
      return { ...prev, modules: newModules };
    });
    setIsDirty(true);
  ...
  if (courseId !== 'new' && !isAuthLoading && user?.role !== 'ADMINISTRATOR' && user?.id !== course.instructorId) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-80px)] text-center p-4">
        ...
        <Link href="/manage-courses" className={buttonVariants({ variant: "outline" })}>Volver</Link>
      </div>
    );
  }
    ...
    <div className="space-y-4 pb-24">
      <header className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 border-b bg-background sticky top-[63px] sm:top-0 z-20 -mx-4 md:-mx-6 lg:-mx-8 px-4 md:px-6 lg:px-8">
        <div className="flex items-center gap-4 w-full sm:w-auto">
          <Button asChild variant="outline" type="button" size="sm" className="shrink-0">
            <Link href="/manage-courses"><ArrowLeft className="mr-2 h-4 w-4" /> Volver</Link>
          </Button>
          <h1 className="text-xl font-semibold truncate">{courseId === 'new' ? 'Crear Nuevo Curso' : 'Editar Curso'}</h1>
        </div>
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <Button asChild variant="secondary" type="button" className="w-full sm:w-auto">
            <Link href={`/courses/${courseId}`} target="_blank"><Eye className="mr-2 h-4 w-4" /> Vista Previa</Link>
          </Button>
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Detalles básicos y descripción.</CardDescription>
            </CardHeader>
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Controla la visibilidad y el estado del curso.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                ...
                <div>
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        variant={"outline"}
                        className={cn("w-full justify-start text-left font-normal", !course.publicationDate && "text-muted-foreground")}
                        disabled={isSaving}
                      >
                        <CalendarIcon className="mr-2 h-4 w-4" />
                        {course.publicationDate
                          ? format(new Date(course.publicationDate), "PPP", { locale: es })
              ...
              {course.imageUrl ? (
                <div className="relative aspect-video rounded-md overflow-hidden border w-full">
                  <Image
                    ...
                    onError={() => updateCourseField('imageUrl', null)}
                  ...
                  <div className="absolute top-2 right-2 z-10 flex gap-1">
                    ...
                    <Button
                      ...
                      size="icon"
                      className="rounded-full h-8 w-8"
                      onClick={() => updateCourseField('imageUrl', null)}
                      disabled={isSaving}
                    >
                ...
                <Label
                  ...
                  className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed rounded-md cursor-pointer bg-muted/20 hover:bg-muted/30 transition-colors"
                >
      ...
      <div className="fixed bottom-0 left-0 md:left-72 group-data-[collapsed=true]:md:left-20 right-0 bg-background/95 backdrop-blur-sm border-t p-4 z-20">
        <div className="max-w-screen-2xl mx-auto flex flex-col sm:flex-row justify-end gap-2">
          ...
          >
            ...
            {isSaving ? 'Guardando...' : (courseId === 'new' ? 'Crear y Guardar' : 'Guardar Cambios')}
          </Button>
        </div>
      </div>
      ...
      <AlertDialog open={!!itemToDeleteDetails} onOpenChange={(isOpen) => !isOpen && setItemToDeleteDetails(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ¿Estás seguro? Esta acción eliminará "{itemToDeleteDetails?.name}" y su contenido.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => { itemToDeleteDetails.onDelete(); setItemToDeleteDetails(null) }}
...
const BlockTypeSelector = ({ onSelect }) => (
  <DropdownMenu>
    ...
    <DropdownMenuContent>
      <DropdownMenuItem onSelect={() => onSelect('TEXT')}>
        <FileText className="mr-2 h-4 w-4" />
        Texto/Enlace
      </DropdownMenuItem>
      <DropdownMenuItem onSelect={() => onSelect('VIDEO')}>
        <Video className="mr-2 h-4 w-4" />
        Video (YouTube)
      </DropdownMenuItem>
      <DropdownMenuItem onSelect={() => onSelect('FILE')}>
        <FileGenericIcon className="mr-2 h-4 w-4" />
        Archivo (PDF, Img)
      </DropdownMenuItem>
      <DropdownMenuItem onSelect={() => onSelect('QUIZ')}>
