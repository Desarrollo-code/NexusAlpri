...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const courseIds = baseMessages
            .filter(msg => msg.triggerType.startsWith('COURSE_') && msg.triggerId)
        ...
        const enrichedMessages: EnrichedMotivationalMessage[] = baseMessages.map(message => {
            ...
            // Solo buscar en el mapa si el trigger es de tipo curso y el ID existe.
            if (message.triggerType.startsWith('COURSE_') && message.triggerId && coursesMap.has(message.triggerId)) {
                triggerCourse = coursesMap.get(message.triggerId) || null;
            }
            // Para niveles, el `triggerId` ya es el identificador (ej. 'level-5')
            // y no necesita una búsqueda adicional en la DB de cursos.
            else if (message.triggerType === 'LEVEL_UP' && message.triggerId) {
                // Para mantener la consistencia del objeto, creamos un objeto similar a curso
                 triggerCourse = {
                    id: message.triggerId,
                    title: `Nivel ${message.triggerId.split('-')[1] || ''}`
                };
            }
            return {
                ...message,
