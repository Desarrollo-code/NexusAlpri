...
CREATE TABLE `PlatformSettings` (
    ...
    `resourceCategories` VARCHAR(191) NOT NULL DEFAULT 'Recursos Humanos,TI y Seguridad,Marketing,Ventas,Legal,Operaciones,Finanzas,Formación Interna,Documentación de Producto,General',
...
CREATE TABLE `LessonTemplate` (
    ...
    `sourceLessonId` VARCHAR(191) NULL,

    UNIQUE INDEX `LessonTemplate_sourceLessonId_key`(`sourceLessonId`),
...
CREATE TABLE `TemplateBlock` (
    ...
    `type` ENUM('TEXT', 'VIDEO', 'QUIZ', 'FILE') NOT NULL,
    `order` INTEGER NOT NULL,
    `lessonTemplateId` VARCHAR(191) NOT NULL,

    INDEX `TemplateBlock_lessonTemplateId_idx`(`lessonTemplateId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
